...
%依此类推。
function [VC,NV,TD,Q_U,w_pp]=decode(route_k,cap,demands,a,b,L,s,dist)
...
for i=1:cusnum
    ...
    [flag,Q_U(i),wait,w_pp]=JudgeRoute(preroute,cap,demands,a,b,L,s,dist);%判断当前路径是否满足时间窗约束和载重量约束，0表示违反约束，1表示满足全部约束
    if flag==1
        %如果满足约束，则更新车辆配送方案VC
        VC{count}=preroute;               
    else
